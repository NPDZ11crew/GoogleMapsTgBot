...
async def handle_place_type(update: Update, context: ContextTypes.DEFAULT_TYPE, SUBTYPE: int = 2):
    query = update.callback_query
    await query.answer()
    context.user_data['type'] = query.data

    # Подтипы по типу - сделать адекватно
    subtypes = {
        "0": [
        ...
        "14": [
            ...
            "electrician", "florist", "food_delivery", "foot_care", "funeral_home",
            "hair_care", "hair_salon", "insurance_agency", "laundry", "lawyer",
            "locksmith", "makeup_artist", "moving_company", "nail_salon", "painter",
        ...
        "16": [
            ...
            "sports_activity_location", "sports_club", "sports_coaching", "sports_complex", "stadium",
            "swimming_pool"
        ],
        "17": [
            "airport", "airstrip", "bus_station", "bus_stop", "ferry_terminal",
            "heliport", "international_airport", "light_rail_station", "park_and_ride", "subway_station",
            "taxi_stand", "train_station", "transit_depot", "transit_station", "truck_stop"
        ]
    }

    buttons = [
        [InlineKeyboardButton(st, callback_data=st)]
        for st in subtypes.get(query.data, [])
    ]
    reply_markup = InlineKeyboardMarkup(buttons)
    await query.edit_message_text("Выбери подтип заведения:", reply_markup=reply_markup)
    return SUBTYPE
